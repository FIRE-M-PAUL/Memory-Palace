...
function buildProfile(tier: ViewportTier, isReducedMotion: boolean): PerformanceProfile {
  ...
  if (isMobile) {
    return {
      ...
      ringScale: 1.28,
      nodeHitScale: 2.5,
      nodeScaleMultiplier: 0.92,
      coreWireScale: 0.42,
      coreInnerScale: 0.24,
      coreLabelDistance: 12,
      sceneLiftY: 0.62,
      mobileTouchSelectOnly: true,
      cameraDefault: [0, 2.8, 9.8],
      cameraFov: 49,
      orbitMinDistance: 4.6,
      orbitMaxDistance: 13.8,
      orbitMaxDistanceFocused: 11.4,
      orbitDamping: 0.18,
      rotateSpeed: 0.34,
      enablePan: false,
      showPedestals: false,
      ...
      floorRadius: 4.5,
      fogFar: 24,
      orbitRingSpeed: !isReducedMotion ? 0.013 : 0,
    };
  }
